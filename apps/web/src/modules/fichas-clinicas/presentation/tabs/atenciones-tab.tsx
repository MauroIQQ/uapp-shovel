...
import { ChevronDown, ChevronUp, Pencil, Plus, Stethoscope } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-lg">Atenciones ({data.length})</h2>
        <Button
          onClick={() => {
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm">No hay atenciones registradas</p>
        ...
        <div className="relative pl-6 before:absolute before:top-2 before:left-2.5 before:h-[calc(100%-16px)] before:w-px before:bg-border">
          {sorted.map((b) => {
            const isExpanded = expanded[b.id];
            return (
              <div key={b.id} className="relative mb-4 last:mb-0">
                <div className="absolute top-3 -left-[19px] size-3 rounded-full border-2 border-primary bg-background" />
                <Card>
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <div>
                        <CardTitle className="text-base">{b.motivo_consulta}</CardTitle>
                        <p className="text-muted-foreground text-xs">
                          {b.created ? format(new Date(b.created), "dd/MM/yyyy HH:mm") : "Sin fecha"}
                        </p>
                      </div>
                    ...
                    {isExpanded && (
                      <div className="space-y-3 text-sm">
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          {b.pulso && <Badge variant="outline">Pulso: {b.pulso} lpm</Badge>}
                          {b.frecuencia_respiratoria && (
                            <Badge variant="outline">FR: {b.frecuencia_respiratoria} rpm</Badge>
                          )}
                          {b.temperatura && <Badge variant="outline">Temp: {b.temperatura} °C</Badge>}
                          {b.saturacion_o2 && <Badge variant="outline">SatO₂: {b.saturacion_o2}%</Badge>}
                          {b.glicemia && <Badge variant="outline">Glicemia: {b.glicemia} mg/dL</Badge>}
                          {b.peso && <Badge variant="outline">Peso: {b.peso} kg</Badge>}
                          {b.talla && <Badge variant="outline">Talla: {b.talla} cm</Badge>}
                          {b.imc && <Badge variant="outline">IMC: {b.imc}</Badge>}
                          {b.perimetro_abdominal && <Badge variant="outline">PA: {b.perimetro_abdominal} cm</Badge>}
                        </div>
                        {b.plan_terapeutico && (
                          <div>
                            ...
                            <ul className="mt-1 space-y-2">
                              {b.recetas.map((r) => (
                                <li key={r.id}>
                                  ...
                                  {r.observaciones && (
                                    <p className="text-muted-foreground text-xs">{r.observaciones}</p>
                                  )}
                                </li>
                              ))}
                            </ul>
